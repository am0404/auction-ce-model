...
def cmd_qb_convergence(args) -> int:
    ...
    from pathlib import Path as _P
    from .qbconvergence_experiment import run
    ...
    d = _P(args.out_dir)
    d.mkdir(parents=True, exist_ok=True)
    (d / "qb_convergence.json").write_text(
        json.dumps(local, indent=2, default=str), encoding="utf-8")
    if args.json_out:
        _write_json(args.json_out, blob)
    print()
    c = blob["convergence"]
    print(f"  union {c['status']} after {c['n_rungs_completed']} rungs "
          f"(ladder {c['ladder_fingerprint']})")
    print(f"  frontier: {blob['frontier']['classification']}")
    print(f"  player-level output -> {d / 'qb_convergence.json'} (IGNORED)")
...
def add_tactical_parser(sub) -> None:
    ...
    s = inner.add_parser(
        "qb-convergence",
        help="push the QB full-budget union to a convergence stopping rule "
             "and gate the SIMULATED frontier on it")
    s.add_argument("--contract",
                   default="local_data/real_player_contract_v1.json")
    s.add_argument("--sleeper-csv",
                   default="local_data/sleeper_2qb_values_2026_clean.csv")
    s.add_argument("--out-dir", default="local_data/tactical")
    s.add_argument("--json-out", default=None,
                   help="sanitized aggregate (safe to commit)")
